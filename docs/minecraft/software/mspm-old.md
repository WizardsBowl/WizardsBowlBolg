---
title: 旧版MC音效包制作工具发布&下载，简易图形交互界面，Java版&基岩版均可用
description: 一个简单的GUI小工具，用于给Minecraft制作Java版或基岩版的音效资源包
head:
  - - meta
    - name: keywords
      content: Minecraft,我的世界,资源包,音效包,GUI,小工具,生成,制作,MC音效包制作工具,发布
tags: [Minecraft,软件,个性化]
---

# 旧版MC音效包制作工具发布&下载，简易图形交互界面，Java版&基岩版均可用

简单的小工具，能够使用GUI快速生成JE/BE版的音效包（本质上就是一个基本上只存放了音频文件的资源包）。

已经停更...多久来着？

[下载链接](#latest)

::: tip GitHub仓库地址
[WizardsBowl/MCSoundPackMaker: 一个使用GUI生成MC音效包的小工具。](https://github.com/WizardsBowl/MCSoundPackMaker)
:::

## B站视频

<BiliVideo src="//player.bilibili.com/player.html?isOutside=true&aid=113153799096389&bvid=BV1eWtveHEZE&cid=25901729070&p=1" />

## 旧版停更说明

一开始我是打算用.NET 8/9/10重构项目的。@@但是拖了很久都只有新建文件夹@@

等下次更新时会把包括音效包在内的多个功能合并为另一个新软件发布。（暂定）

## 新版链接

[新建文件夹](#旧版停更说明)

## 历史版本

### v1.0.2.0 <Badge type="tip" text="latest" id="latest" />

日期：2025/4/5

更新说明：

1. 单个声音事件使用多个音频文件时允许为每个音频文件单独设置权重、音量及音调。
2. 移除未使用的菜单项，移除不必要的快捷键。
3. 现在保存项目文件时使用UTF-8编码字符串。

<DownloadCounter fileName="MC音效包制作工具v1.0.2.0.rar" filePath="https://file.wizardsbowl.com/minecraft/mspm-old/MC%E9%9F%B3%E6%95%88%E5%8C%85%E5%88%B6%E4%BD%9C%E5%B7%A5%E5%85%B7v1.0.2.0.rar" />

---

### v1.0.1.0

日期：2025/3/23

更新说明：

1. 新增支持单个声音事件使用多个音频文件。
2. 允许修改Java版资源包格式版本。

<DownloadCounter fileName="MC音效包制作工具v1.0.1.0.rar" filePath="https://file.wizardsbowl.com/minecraft/mspm-old/MC%E9%9F%B3%E6%95%88%E5%8C%85%E5%88%B6%E4%BD%9C%E5%B7%A5%E5%85%B7v1.0.1.0.rar" />

---

### v1.0.0.1

日期：2024/10/1

更新说明：

1. 修正一些UI问题。
2. 新增一些异常处理。

<DownloadCounter fileName="MC音效包制作工具v1.0.0.1.rar" filePath="https://file.wizardsbowl.com/minecraft/mspm-old/MC%E9%9F%B3%E6%95%88%E5%8C%85%E5%88%B6%E4%BD%9C%E5%B7%A5%E5%85%B7v1.0.0.1.rar" />

---

### v1.0.0.0

日期：2024/9/17

更新说明：

1. 实现基本功能。

<DownloadCounter fileName="MC音效包制作工具v1.0.0.0.rar" filePath="https://file.wizardsbowl.com/minecraft/mspm-old/MC%E9%9F%B3%E6%95%88%E5%8C%85%E5%88%B6%E4%BD%9C%E5%B7%A5%E5%85%B7v1.0.0.0.rar" />